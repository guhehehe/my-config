...
if s:Enabled("g:python_highlight_builtin_funcs")
  if s:Python2Syntax()
    syn keyword pythonBuiltinFunc	apply basestring buffer callable coerce contained
    syn keyword pythonBuiltinFunc	execfile file help intern long raw_input contained
    syn keyword pythonBuiltinFunc	reduce reload unichr unicode xrange contained
    if s:Enabled("g:python_print_as_function")
  ...
  syn keyword pythonBuiltinFunc	__import__ abs all any contained
  syn keyword pythonBuiltinFunc	bin bool bytearray bytes contained
  syn keyword pythonBuiltinFunc	chr classmethod cmp compile complex contained
  syn keyword pythonBuiltinFunc	delattr dict dir divmod enumerate eval contained
  syn keyword pythonBuiltinFunc	filter float format frozenset getattr contained
  syn keyword pythonBuiltinFunc	globals hasattr hash hex id contained
  syn keyword pythonBuiltinFunc	input int isinstance long contained
  syn keyword pythonBuiltinFunc	issubclass iter len list locals map max contained
  syn keyword pythonBuiltinFunc	min next object oct open ord contained
  syn keyword pythonBuiltinFunc	pow property range contained
  syn keyword pythonBuiltinFunc	repr reversed round set setattr contained
  syn keyword pythonBuiltinFunc	slice sorted staticmethod str sum super tuple contained
  syn keyword pythonBuiltinFunc	type vars zip contained
endif
...
if version >= 508 || !exists("did_python_syn_inits")
  ...
  HiLink pythonBoolean          Identifier
...
syn match pythonParameters "[^,]*" contained contains=pythonParam,pythonBrackets,pythonClsSelf skipwhite
...
" for doc strings
syn region pythonDocStr start=+\%("""\|'''\)+ end=+\%("""\|'''\)+

" for cls and self
" syn keyword pythonClsSelf cls self
syn keyword pythonClsSelf self cls

" for references
syn region pythonParIpt start='from ' end=' ' keepend contains=pythonParIptContent,pythonImport
syn match pythonParIptContent ' .\+ ' contained
syn match pythonRef 
            \ '\%(^\| \|=\|==\|(\|\[\|{\|+\|-\|*\|/\|<\|>\|%\)[_a-zA-Z]\+\.' 
            \ contains=pythonObjRef,pythonOperator,pythonExtraOperator,pythonExtraPseudoOperator,pythonClsSelf
syn match pythonObjRef '[_a-zA-Z]\+' contained

" for builtin funcs
syn match pythonBuiltinF 
            \ '\%(^\| \|=\|==\|(\|\[\|{\|+\|-\|*\|/\|<\|>\|%\)[_a-zA-Z]\+(' 
            \ contains=pythonBuiltinFunc, pythonRef
...
if version >= 508 || !exists("did_python_syn_inits")
  ...
  "
  HiLink pythonFunction		Function
  HiLink pythonExtraOperator cusPythonOperator
  HiLink pythonExtraPseudoOperator cusPythonOperator
  HiLink pythonClass Normal
  HiLink pythonParameters cusPythonFuncArgs
  HiLink pythonParam Normal
  HiLink pythonClassParameters InheritUnderlined
  HiLink pythonDocStr Comment
  HiLink pythonClsSelf cusPythonClsSelf
  HiLink pythonRef Normal
  HiLink pythonObjRef cusPythonObjRef
